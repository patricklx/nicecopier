...
NiceCopier::NiceCopier(bool hideIcon) :
	QWidget(NULL),
	ui(new Ui::NiceCopier)
{
	...
	taskbar = new MyTaskBarIcon(this, hideIcon);
	...
	if(dir.exists()){
		...
		foreach(QString fname, list){
			QFile f("Tasks/"+fname);
			qDebug() << fname;
			qDebug() << settingsDir.absolutePath()+"/"+fname;
			if(f.copy(settingsDir.absolutePath()+"/"+fname)){
				f.remove();
			}
		}
		dir.cdUp();
...
{
	QString dir = NcSettings::getPath();

	QFile file(dir+"/Tasks/tasklist.nc");
	if(file.open(QFile::ReadOnly))
	{
		while(!file.atEnd())
		{
			QString task(file.readLine());
			task = dir + "/" + task.trimmed();
