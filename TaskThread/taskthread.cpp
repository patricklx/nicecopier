	...
	: QThread(),fileCopyQueue(this)
{
	...
	{
		qSort(sourceList.begin(), sourceList.end());
        if( config==ParseAndCopy  && !started){
...
void TaskThread::saveAllInfo()
{
	...
	doc.writeStartDocument();
	{
		doc.writeStartElement("TASK_INFO");
		{
			...
			doc.writeAttribute( "STARTED", started );
			doc.writeAttribute( "MD5CHECK", checkMd5);
			if ( prepareDone )
			{
				doc.writeAttribute( "SIZE", totalSize );
				doc.writeAttribute( "SIZE_DONE", fileSizeToSave );
				doc.writeAttribute( "TOTAL_FILES", totalFiles );
			}
...
int TaskThread::loadFromXML( QIODevice &device, bool no_save  )
{
	...
	checkMd5 = QVariant(attr.value("MD5CHECK").toString()).toBool();
